...
public class ShantayShop extends ComponentCentered {

    private static final int ORANGE_TEXT_COLOR = -26593;
    private static final Rectangle TITLE_BOUNDS = new Rectangle(0, 0, 488, 35);
    private static final String TITLE_TEXT = "Shantay Pass Shop";

    public ShantayShop(ScriptCore core) {
        super(core);
    }
    ...
    public boolean close() {
        ...
        boolean closed = core.pollFramesHuman(() -> !isVisible(), RandomUtils.uniformRandom(4000, 8000));
        core.log(getClass(), closed ? "TargetView closed." : "Timeout waiting for TargetView to close.");
        return closed;
    }

    public Rectangle getWaterskinBuyArea() {
        Rectangle bounds = getBounds();
        if (bounds == null) {
            return null;
        }
    ...
    public Rectangle getShantayPassBuyArea() {
        Rectangle bounds = getBounds();
        if (bounds == null) {
            return null;
        }
